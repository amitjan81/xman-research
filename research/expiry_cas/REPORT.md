...
The short version: **the next-week wings did reprice off the indicative, monotonically in moneyness, and the magnitude is the finding.** The deepest listed put (74,700 PE, moneyness 0.96) reached **+76.7 %** through 15:18–15:23 and **+66.8 %** in its largest single traded minute. But the move is **₹11.60 → ₹19.35** — ₹7.75 in total. **The Nifty comparison belongs in rupees, not in a ratio:** the same-moneyness Nifty front put moved ₹1.60 → ₹1.70, nominally 12× smaller in percentage terms but literally one tick on a ₹1.60 option, and a ratio of two such percentages is a ratio of ticks. The comparison that carries weight is the 12-day Nifty put at a comparable ₹8.00 premium, which moved ₹0.25 against the Sensex wing's ₹7.75 — an order of magnitude, inside a few rupees. Selling the largest spike on the board and covering at 15:39 returns **₹92 per lot** against an adverse same-session mark at the band floor of **₹8,350** — about **1 : 100**, on the most flattering denominator available, and the band bounds *today's close* while the contract carries the overnight gap, so the short has **no worst case at all**. Across 8 control sessions no wing put moved ≥100 %; neither did any on the event session, so that trigger has no false positives and no true positives. The event separates from an ordinary day only at a 25–50 % threshold, which on these premiums is a few rupees.

**The stronger result is the one that kills the trade.** At the indicative low of 74,983.19 every strike above it was notionally in the money, yet the wings traded **hundreds of rupees below that intrinsic** at every strike in every minute. The mispricing the thesis needs — wings marked to a crash that was not going to happen — is not present to be traded. The market never made the error.
