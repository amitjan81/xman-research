...
The short version: the divergence is **small, late, and non-reverting**. During the indicative crash itself (15:18–15:23) the Sensex option-implied index barely moved — `d` ran **+0.28 to −44.93 points (≤0.06 %)**, about **2 %** of the 2,199.72-point indicative drop. `d` peaked at **−276.08 points (−0.36 %)** only at **15:27**, four minutes after the indicative had recovered, and **2026-08-04 — an ordinary Nifty expiry — diverged more (−379.49)**. The largest |d| anywhere in the corpus is **0.4818 %**, so the ≥0.5 % trigger the owner's strategies need **never fires on 08-27 at any β** from β̂−SE to 1 (it fires once at β = 1, on 08-04). Median closure by 15:35 across the four largest sessions is **17.0 %**. The band-bounded wing trade collects 3.80 points gross against a 1,274.97-point capped loss, and no listed strike lies outside the ±3 % band — the 75,000 PE that moved is 900 points below the lowest strike in the corpus and has no bar here, so its overpricing can be tested neither way.

## 0.5 The wing test on the ±25 chain — `WINGS_0827.md`

§0.4 closes with the ±10 ladder's binding limit: the strikes the thesis points at are below the lowest strike in that corpus, so the wing claim can be tested neither way. A separate ±25 chain tree — every listed strike **74,700–79,700** for 2026-08-27 — removes that limit for the **2026-09-03** expiry, and **`research/expiry_cas/WINGS_0827.md`** is the test it makes possible. `wings.py` generates it for any underlying and session with a chain tree.

**Read the substitution before reading the result.** The 27 Aug expiring series has refdata but **zero bars** in the chain tree — it delisted into settlement — so the ~4,800 % move remains untestable on any feed here. The 75,000 PE examined is the **3 Sep** contract: a week of life left, an unbounded overnight gap, and the same strike by coincidence.

The short version: **the wings repriced as insurance getting dearer, not as a mark to the indicative — and that distinction is the whole result.** The largest wing mover, 74,700 PE, went **₹11.60 → ₹19.35** at 15:21 (+66.8 %). But the traded forward fell only **47.35 points** through the crash window — **2 %** of the indicative's 2,199.72 — and an empirical delta fitted from this chain attributes just ₹1–3 of each ₹5–7 wing move to that forward change. The rest is a volatility and skew bid. Nothing priced the indicative as a level.

**Three things the ±25 tree settles that the ±10 ladder could not.** First, the options forward sits **289 points above cash** at 15:14 (77,470.15 parity vs 77,181.61 feed), of which carry at 6.5 % explains only 96 — so moneyness, the wing cut and the at-the-money time-value strike are all struck on **cash**, and parity supplies only movement. Second, only **1 of the session's 4** 25 %-plus wing moves landed inside 15:18–15:23; the other three landed at 15:30–15:31, around the close publication, which an ordinary session also produces. Third, the Nifty comparison belongs in rupees: the same-moneyness Nifty put moved ₹1.40 → ₹1.45, one tick on a near-worthless contract, so a ratio of percentages there is a ratio of ticks.

**There is a repricing here; there is no mispricing.** Fading the headline spike — sell 74,700 PE at ₹19.35 at 15:21, cover at ₹14.75 at 15:39 — returns **₹92 per lot** against an adverse same-session mark of **₹5,641**: **1 : 61**, and only **2 lots** printed in that entry minute. The band bounds today's auction close, not a 3 Sep contract's overnight gap, so the short has no worst case at all. The few rupees of volatility premium that did appear were largely justified — the close came in 249 points below the 15:15 reference.
