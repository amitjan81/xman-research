...
Facts held constant: strike step **100**, lot size **20** (read from each session's own refdata), 29 strikes on the ladder, `minute_ts` in microseconds. **The `SENSEX` index symbol stops printing at 15:29 on every session**, so the entire auction window's underlying is derived from the ATM pair by put-call parity, `S = C − P + K`. The feed and the parity series are **never mixed inside one range or difference** — see §4.
...
**Reading it.** The auction window is flat to **within one point** on every Sensex expiry, against 82.85–474.00 points in the preceding half hour of the same sessions — a ratio of **100× to 1,350×**. The controls show the half hour is not simply always calm: on a non-expiry day the auction window carries 45–59 points of proxy noise, which is what the *absence* of a pinned settlement looks like.
...
**Then it stops dead.** Across 15:30–15:39 the largest single minute is **0.80 points** and the total range is **0.95**. Per minute, the auction window's movement is **1/243rd** of the preceding ten minutes'.

**Two corroborations that the number was already fixed at 15:29.** First, the index feed — which has run a median **−54.71 points** away from the parity level all day (§4) — prints **76,933.59** at 15:29 against a parity level of **76,933.60**: agreement to **0.01 of a point**, after disagreeing by tens of points on every fresh minute before it. The same convergence holds on the other three post-CAS expiries (|feed − parity| at 15:29 = 0.29 / 0.61 / 0.42). Second, every option in the window prices at exact intrinsic against that number — CE 76900 → 33.50 (intrinsic 33.60), PE 77000 → 66.30 (66.40), PE 77100 → 166.30 (166.40), and every call at 77,000 and above and every put at 76,900 and below → 0.10.

**The auction window is not empty — it is enormous and it is uniform.** **416 of 420** strike-minutes trade with volume in 15:30–15:39. The volume field decays monotonically from the 15:30 bar to the 15:39 bar in a ratio of **39.6 : 1**, with **~38% of the window's total in its first minute**. (Only the shape is claimed; the absolute vendor volume figures are left uninterpreted.) A window with very heavy participation, complete price agreement, and nothing to arbitrage — the opposite of the dislocation the strategy thesis needs.
...
| butterfly CE | median 0.00, 5.3–6.2% > cost, 4–8 persistent | median 0.00, **0.0% > cost**, **0 persistent** |
| butterfly PE | median 0.00, 2.6–6.7% > cost, 0–6 persistent | median 0.00, 0–0.6% > cost, **0 persistent** |
...
**The cost threshold is a per-unit, one-lot figure and is not size-invariant.** It runs **5.78–7.79 points** on Sensex against **1.8–2.5** on Nifty. That gap is **not a market difference** — it is ₹20/order brokerage spread over a lot of **20** units instead of **65**. Larger orders lower it proportionally and every `% > cost` rises accordingly. The threshold is applied strictly (`> cost`, not `≥`).

**Costs, and the one substitution made.** The repo's statutory stack (`src/xman_research/backtest/costs.py`) carries no BSE-specific transaction-charge schedule, so **`nse.transaction_charge.options` is used for the BSE legs**. Everything else in the stack — STT (including exercise STT at 0.15% of intrinsic from 2026-04-01), stamp duty, GST, SEBI turnover fee, brokerage — is statutory or broker-side and exchange-independent. The exchange transaction charge is a small share of a threshold that per-order brokerage dominates, so the substitution moves the threshold by a fraction of a point; **the direction of the error is unknown**, since BSE's own rate is not established here.
...
| Date | strikes | median return | to zero (≤ −95%) | best | multibag (≥ +100%) |
|---|---|---|---|---|---|
...
| CE strike | 15:25 → 15:39 | | PE strike | 15:25 → 15:39 |
|---|---|---|---|---|
| 76200 | 952.2 → 732.8 (−23%) | | 76200–76900 | 4.8–30.2 → **0.1 (−99 to −100%)** |
| 76500 | 650.1 → 432.8 (−33%) | | 77000 | 52.4 → 66.3 (**+27%**) |
| 76800 | 347.2 → 133.3 (−62%) | | **77100** | 76.8 → 166.3 (**+116%**) |
| 76900 | 269.9 → 33.5 (−88%) | | **77200** | 126.3 → 265.9 (**+110%**) |
| **77000–77900** | 182.3–2.1 → **0.1 (−98 to −100%)** | | 77300 | 200.0 → 367.0 (+84%) |
| | | | 77900 | 782.5 → 971.6 (+24%) |

**Half the ladder went to zero and two strikes more than doubled, in fourteen minutes.** That is the "wild swing" the owner saw. It is **neither volatility nor arbitrage** — it is the deterministic collapse of every option to |S−K| against a settlement value already determined by 15:29. Every exit price above is intrinsic to within 0.1. There was no dispersion to trade.
...
| b | Sell ATM straddle 15:00 → settlement | post-CAS **+5,337 / −2,612 / +1,572 / −1,578** (sum **+2,719**); pre-CAS −2,619 / −268 / −3,152; all seven **−₹3,320** | 2/4 post, **2/7** overall | **−₹3,152** | **No** — see below |
...
**What survives on Sensex, as on Nifty.** The one repeatable structure is the **premium crush into 15:29** — ATM time value falls from 31–341 points at 15:00 to ≈0.1. Strategy (b) fails to harvest it only because a *fixed* ATM strike is short gamma. A **delta-neutral or strike-following** variant, or an iron-fly capping the 08-06/08-27 tail, is the one candidate worth a real backtest. That is a claim about 15:00–15:29, not about the auction window.

**A mechanism the bars are consistent with.** Exercise STT falls on the **long** at 0.15% of intrinsic, so an ITM long has a direct incentive to sell at intrinsic in 15:30–15:39 rather than be exercised — which would explain both the very heavy auction-window volume (416/420 strike-minutes trading) and the exact-intrinsic pin. **A hypothesis the corpus is consistent with, not a tested finding.**
...
**No. Once the index level is divided out the two are the same market to within noise, and the auction window is if anything tighter on Sensex.**
...
| Auction-window range, expiry | 0.25–0.95 | 0.10–1.15 | 1.2e-5 % vs 4.7e-6 % |
...
- **One source per series, always.** The feed freezes (~23–24 of 40 minutes post-CAS on both underlyings; on Sensex the index symbol stops entirely at 15:29), and parity sits tens of points from it. A movement series assembled from whichever source looked better each minute reports that offset as volatility. Every series here is the **anchor pair's parity level** throughout; the feed appears only in its own labelled column and is never differenced against parity. An earlier cut of the Nifty pass **spliced the two**, manufacturing a −136.75-point "1-minute move" on 08-26 and inflating control ranges 2–4× (169.60 where the single-source figure is 40.10).
...
- **The volume field is uninterpreted.** Only its shape (monotone decay, 39.6:1 across the Sensex 08-27 auction window) and traded-bar counts (416/420) are claimed.
