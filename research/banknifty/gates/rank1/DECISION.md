# BANKNIFTY stage two, rank 1 — NOT_EVALUABLE

`short_atm_straddle_hold_n[hold_sessions=1.0,target_notional=5000000.0] (in-sample)` — short ATM straddle, hold 1 session.
...
> 80.6% of 566 intents could not have been filled (limit 10.0%); the P&L describes trades the market would not have taken

The pre-registered feasibility bar is `max_infeasible_fraction = 0.10`, in
`research/banknifty/gate_v1.toml`, committed before any instance was graded. At 80.6% the
...
| quantity | value |
|---|---|
| deflated Sharpe | 0.2701 (bar 0.90, not applied) |
| annualised Sharpe | 1.590 |
| trial family size at deflation | 109 |
| return observations | 352 |

`109` is the 107 trials the stage-one screen logged against this hypothesis's family, plus
