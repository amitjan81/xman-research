# BANKNIFTY stage two, rank 8 — NOT_EVALUABLE

`short_atm_straddle_ema_atr_band[ema_band_threshold=1.0,hold_sessions=1.0,target_notional=5000000.0] (in-sample)` — short ATM straddle conditioned on an EMA/ATR band of 1.0, hold 1 session.
...
> 67.6% of 148 intents could not have been filled (limit 10.0%); the P&L describes trades the market would not have taken

The pre-registered feasibility bar is `max_infeasible_fraction = 0.10`, in
`research/banknifty/gate_v1.toml`, committed before any instance was graded. At 67.6% the
...
| quantity | value |
|---|---|
| deflated Sharpe | 0.1164 (bar 0.90, not applied) |
| annualised Sharpe | 1.080 |
| trial family size at deflation | 113 |
| return observations | 352 |

`113` is the 107 trials the stage-one screen logged against this hypothesis's family, plus
