# BANKNIFTY stage two, rank 2 — NOT_EVALUABLE

`short_atm_straddle_post_gap[gap_sigmas=1.5,hold_sessions=3.0,target_notional=5000000.0] (in-sample)` — short ATM straddle entered after a 1.5-sigma gap, hold 3 sessions.
...
> 69.0% of 58 intents could not have been filled (limit 10.0%); the P&L describes trades the market would not have taken

The pre-registered feasibility bar is `max_infeasible_fraction = 0.10`, in
`research/banknifty/gate_v1.toml`, committed before any instance was graded. At 69.0% the
...
| quantity | value |
|---|---|
| deflated Sharpe | 0.7875 (bar 0.90, not applied) |
| annualised Sharpe | 2.477 |
| trial family size at deflation | 111 |
| return observations | 352 |

`111` is the 107 trials the stage-one screen logged against this hypothesis's family, plus
