...
| **Status** | **NOT RUNNABLE ON THIS CORPUS.** Structural, not a bug (§1). No re-validation may be attempted until §1.3 is satisfied. |
| Approval | The §1 blocker and the §7 alpha reframing are **awaiting owner approval**. |
...
### 1.2 Why no guard is acceptable
...
This is categorically worse than §5's ~43% unobservable gaps. That is a limit on **power**. This is a limit on **validity**.

### 1.3 The two conditions under which M2 becomes runnable
...
Until one holds, M2 is not re-run. **Every attempt burns a family trial and permanently deflates every future member of the family** (§9), for no result.
...
**Both arms require a round trip in a named strike**, which is what §1 blocks. M1 is exposed to the same moving ladder and survives it, because settlement is on $S_T$ and needs no listed contract — a strike leaving the ladder costs M1 a stale mark, and costs M2 the run.
...
- $K_t \notin \mathcal{K}_{t+1}$: **the ladder moved** (§1). Not a filter — the engine refuses, and it must not be converted into one.
...
## 6. Stratification — reported, NOT graded ▲
...
## 7. Null hypothesis — alpha over the benchmark, gross ▲
...
1. **The model does not run** (§1). Everything below is conditional on §1.3 being satisfied first.
2. ~43% of candidate gaps are unobservable (§5). Caps the power of any gap-based model on this corpus, independently of §1.
