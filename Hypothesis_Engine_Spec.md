...
| | |
|---|---|
| **Version** | **0.5** (ownership assigned) |
| **Status** | Proposed |
| **Date** | 12 August 2026 |
| **Parents** | FRD v0.2 — `Options_Research_Platform_FRD.md` (requirements baseline) · HLD v0.3 — `Options_Research_Platform_HLD.md` (architecture). Both committed to this repository, so every `FR-*` ID cited here resolves |
...
|---|---|---|
| **0.5** | 12 Aug 2026 | **Ownership assigned; FRD committed.** ENF-FR-04's gross short-gamma cap now names **the xman platform as owner** — accepted by the owner as an xman-side change — and is restated as a declared cross-document dependency rather than an open question, with the ENF-FR-05 fallback made explicitly non-contingent on it landing. Separately, FRD v0.2 is now committed to this repository (`.docx` authoritative, markdown rendering for diff and citation), so the `FR-*` IDs this specification cites resolve against a version-controlled document rather than an uploaded file. |
...
**ENF-FR-04** *(Must, Phase 2, inspection)* — **Owner: the xman platform.** The gross short-gamma cap is a requirement levied on xman, accepted by the owner as an xman-side change to be made there. Computing book gamma requires Greeks. xman possesses Greeks machinery — chain implied-volatility inversion and Greeks on a short production cycle — so the *capability* exists; what remains is **wiring it into a pre-trade risk check on the order path**, which is a different piece of work and is xman's to schedule.

This is a **cross-document dependency**: this specification depends on it, does not implement it, and cannot verify it. Two consequences follow, and both are binding:

1. **The dependency is declared, not assumed.** Until xman confirms the pre-trade check is live, any design statement here that relies on the gamma cap being enforced is provisional, and the ENF-FR-05 fallback is the operative control.
2. **The fallback is not contingent on the dependency landing.** ENF-FR-05 is built regardless, because a control that exists only after another team's change is not a control during the interval that matters most — the beginning.
