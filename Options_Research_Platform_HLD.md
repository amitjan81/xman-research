...
| Metrics Reporter | Standard metric set plus cost-breakeven multiple and tail-aware metrics for negatively-skewed strategies (FR-BT-07/13/14) |
...
**Lineage and reproducibility.** A result's identity is the tuple (snapshot manifest id, signal version hash, parameter hash, cost-config version, engine version) — FR-BT-06 verbatim, implemented as the run record's key. Lineage is walkable in both directions: hypothesis → trials → runs → gate records → deployments → emissions (NFR-02), and every artefact back to the bytes that produced it.
